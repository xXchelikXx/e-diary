...
# Взлом дневника
Дневник можно взломать при помощи __скриптов__. Они находятся в `scripts.py`.

## Нахождение ученика
Для начала работы со скриптами, нужно найти цель — ученика.

Для этого нужна функция `find_schoolkid(schoolkid_name)`.

Если найдено несколько учеников либо никто не найден, функция об этом скажет.

### После этого можно начать использование скриптов.

## Замена оценок
Чтобы поменять все плохие оценки (2,3), можно использовать функцию `fix_marks(schoolkid)`.

Она заменяет __все__ эти оценки на __5__ при помощи `update()`.

## Удаление зачеманий
Чтобы удалить все замечания, можно использовать функцию `remove_chastisements(schoolkid)`.

Она удалит __все замечания__ с помощью аргумента `delete()`.

## Создание похвалы
Если тебе нужно, чтобы у тебя была похвала по какому-либо предмету, используй функию `create_commendation(schoolkid, subject)`.

Здесь передается две переменные. Вторая, `subject`, это название предмета, например "Математика".

Эта функция берет последний урок этого предмета и добавляет случайную похвалу. Весь список можно посмотреть в переменной `commendations` и убрать/добавить новые.
